...
BEGIN_SLOT_MAP(PlaneBehaviorBase)
   ON_SLOT( 1, setSlotCriticalAltitude, Basic::Number )
...
bool PlaneBehaviorBase::setSlotCriticalAltitude(const Basic::Number* const msg)
{
    bool ok = false;
    if (msg != 0)
    {
...
BEGIN_SLOT_MAP(PlaneFire)
   ON_SLOT( 1, setSlotMaxDistance, Basic::Number )
END_SLOT_MAP()
...
bool PlaneFire::setSlotMaxDistance(const Basic::Number* const msg)
{
    bool ok = false;
    if (msg != 0) {
       //double value = Basic::Meters::convertStatic( *msg );
