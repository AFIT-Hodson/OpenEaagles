...
Basic::Ubf::Action* ClipsArbiter::genAction(const Basic::Ubf::State* const state, const LCreal dt)
{
    ...
    {
        //clearBehaviors();
        assertFacts( (Eaagles::xBehaviors::PlaneState*) state );
        clipsCppEnv -> Run();
        getFacts();
        l_timer = 0;
    }
                    ...
                    if( l_fact -> whichDeftemplate -> implied == 0 )
                    {
                        ...
                        for( int k = 0; k < theDeftemplate -> numberOfSlots; k++ )
                        {
                            ...
                            if( slotPtr -> slotName -> bucket == m_behaviorNameBucket )
                            {
                                ...
                                if( l_bhv == 0 )
                                {
                                    l_bhv =  xBehaviors::Factory::createObj( ValueToString( sublist[k].value ) );
                                }
                                if( l_bhv == 0 )
                                {
                                    l_bhv = Lua::Factory::createObj( ValueToString( sublist[k].value ) );
                                }
                            }
                            else
                            {
                                ...
                                switch ( sublist[k].type )
                                {
                                    case FLOAT:
                                    {
                                        l_param = new Basic::Number( ValueToDouble( sublist[k].value ) );
                                        break;
                                    }
                                    case INTEGER:
                                    {
                                        l_param = new Basic::Number( ValueToInteger( sublist[k].value ) );
                                        break;
                                    }
                                    case STRING:
                                    {
                                        l_param = new Basic::String( ValueToString( sublist[k].value ) );
                                        break;
                                    }
                                    case MULTIFIELD:
                                    {
                                        ...
                                        if( l_multifield -> multifieldLength != 0 )
                                        {
                                            ...
                                            for( int l = 0; l < l_multifield -> multifieldLength; l+=2 )
                                            {
                                                switch ( l_multifield -> theFields[ l+1 ] . type )
                                                {
                                                    case FLOAT:
                                                    {
                                                        l_pairobj = new Basic::Number( ValueToDouble( l_multifield -> theFields[ l+1 ] . value ) );
                                                        break;
                                                    }
                                                    case INTEGER:
                                                    {
                                                        l_pairobj = new Basic::Number( ValueToInteger( l_multifield -> theFields[ l+1 ] . value ) );
                                                        break;
                                                    }
                                                    case STRING:
                                                    {
                                                        l_pairobj = new Basic::String( ValueToString( l_multifield -> theFields[ l+1 ] . value ) );
                                                        break;
                                                    }
                                                    default:
                                                    {
                                                ...
                                                {
                                                    l_pair = new Basic::Pair( ValueToString( l_multifield -> theFields[ l ] . value ), l_pairobj );
